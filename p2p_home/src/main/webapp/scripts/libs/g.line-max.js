    ...
    function Linechart(paper, x, y, width, height, valuesx, valuesy, opts) {
        ...
        for (i = 0, ii = valuesy.length; i < ii; i++) {
            ...
            for (var j = 0, jj = valuesy[i].length; j < jj; j++) {
                ...
                (Raphael.is(sym, "array") ? sym[j] : sym) && symset.push(paper[Raphael.is(sym, "array") ? sym[j] : sym](X, Y, (opts.width || 2) * 3).attr({
                    fill: colors[i],
                    stroke: "none"
                }));
        ...
        function createColumns(f) {
            ...
            for (i = 0; i < ii; i++) {
                ...
                f ? (C = {}) : cvrs.push(C = paper.rect(X - 1, y, Math.max(w + 1, 1), height).attr({
                        stroke: "none",
                        fill: "#000",
                        opacity: 0
                    }));
            ...
            for (var i = 0, ii = valuesy.length; i < ii; i++) {
                for (var j = 0, jj = valuesy[i].length; j < jj; j++) {
                    var X = x + gutter + ((valuesx[i] || valuesx[0])[j] - minx) * kx,
                        ...
                        Y = y + height - gutter - (valuesy[i][j] - miny) * ky;
                    f ? (C = {}) : cvrs.push(C = paper.circle(X, Y, Math.abs(nearX - X) / 2).attr({
                            stroke: "#000",
                            fill: "#000",
                            opacity: 1
                        }));
