...
(function () {
    ...
    function Piechart(paper, cx, cy, r, values, opts) {
        ...
        if (len == 1) {
            series.push(paper.circle(cx, cy, r).attr({
            ...
            for (i = 0; i < len; i++) {
                ...
                var p = paper.path(opts.init ? ipath : path).attr({
                    fill: opts.colors && opts.colors[j] || chartinst.colors[j] || "#666",
                    stroke: opts.stroke || opts.colors[j],
                    "stroke-width": (opts.strokewidth == null ? 1 : opts.strokewidth),
        ...
        var legend = function (labels, otherslabel, mark, dir) {
            var x = cx + r + r / 5,
                y = cy,
                h = y + 10,
                holderLayer = $('#holder_layer');

            labels = labels || [];
            dir = (dir && dir.toLowerCase && dir.toLowerCase()) || "east";
            mark = paper[mark && mark.toLowerCase()] || "circle";
            chart.labels = paper.set();
            function chartCount(cp) {
                var chart_covers = chart.covers,
                    chart_series = series,
                    chart_count = null,
                    chart_sector = null;
                for (var k = 0, lk = chart_covers.length; k < lk; k++) {
                    if (chart_covers[k] == cp) {
                        chart_count = k;
                        break;
                    }
                }
                chart_sector = chart_series[chart_count];
                return chart_sector;
            }
            ...
            for (var i = 0; i < len; i++) {
                ...
                chart.labels[i].push(paper[mark](x + 5, h, 5).attr({fill: clr, stroke: "none", 'fill-opacity': '0'}));
                chart.labels[i].push(txt = paper.text(x + 20, h, labels[j] || values[j]).attr(chartinst.txtattr).attr({
                ...
                /*(function(p){
                 var chart_sector = chartCount(covers[p]);
                 covers[p].label[1].hover(
                 function(){
                 chart_sector.stop();
                 chart_sector.scale(1.2, 1.2, chart.cx, chart.cy);
                 covers[p].label[0].stop();
                 covers[p].label[0].attr({ r: 7.5 });
                 this.attr({ "font-weight": 800 });
                 holderLayer.html(covers[p].label[1].attrs.text).css({'top':chart_sector.middle.y,'left':chart_sector.middle.x+r}).show();
                 },
                 function(){
                 this.attr({ "font-weight": 400 });
                 covers[p].label[0].animate({ r: 5 }, 500, "bounce");
                 chart_sector.animate({ transform: 's1 1 ' + covers[p].cx + ' ' + covers[p].cy }, 500, "bounce");
                 holderLayer.hide();
                 }
                 );
                 })(i);*/
